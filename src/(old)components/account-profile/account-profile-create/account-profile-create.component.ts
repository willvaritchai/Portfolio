...
@Component({
  selector: 'account-profile-create',
  templateUrl: './account-profile-create.component.html',
})
export class AccountProfileCreateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (userMsAccount) {
      this.accountFormGroup.get('fullname')?.setValue(userMsAccount.name)
      ...
      this.accountFormGroup.get('email')?.setValue(userMsAccount.username)
    }
